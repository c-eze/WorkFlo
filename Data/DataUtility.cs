...
public static class DataUtility
{
  ...
  public static async Task SeedDefaultProjectMembersAsync(ApplicationDbContext context)
  {
    ...
    //Get Projects
    Project bugTracker = await context.Projects.Include(p => p.Members)
      .FirstOrDefaultAsync(p => p.Id == bugtrackerId);
    Project blog = await context.Projects.Include(p => p.Members)
      .FirstOrDefaultAsync(p => p.Id == blogId);
    Project movie = await context.Projects.Include(p => p.Members)
    ...
    BTUser demopm = await context.Users.FirstOrDefaultAsync(u => u.UserName == "demopm@bugtracker.com");

    BTUser dev1 = await context.Users.FirstOrDefaultAsync(u => u.UserName == "Developer1@bugtracker.com");
    BTUser dev3 = await context.Users.FirstOrDefaultAsync(u => u.UserName == "Developer3@bugtracker.com");
    BTUser dev5 = await context.Users.FirstOrDefaultAsync(u => u.UserName == "Developer5@bugtracker.com");
    BTUser demodev = await context.Users.FirstOrDefaultAsync(u => u.UserName == "demodev@bugtracker.com");

    BTUser sub1 = await context.Users.FirstOrDefaultAsync(u => u.UserName == "Submitter1@bugtracker.com");
    BTUser demosub = await context.Users.FirstOrDefaultAsync(u => u.UserName == "demosub@bugtracker.com");

    await AddUserToProject(pm1, bugTracker, context);
    await AddUserToProject(dev1, bugTracker, context);
    await AddUserToProject(dev3, bugTracker, context);

    await AddUserToProject(pm1, blog, context);
    await AddUserToProject(dev5, blog, context);

    //await AddUserToProject(demopm, movie, context);
    await AddUserToProject(demodev, movie, context);
    await AddUserToProject(demosub, movie, context);
  }

  private static async Task AddUserToProject(BTUser user, Project project, ApplicationDbContext context) {
  ...
  public static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
  {
    ...
    try
    {
      IList<Ticket> tickets = new List<Ticket>() {
                              //PORTFOLIO
                              new Ticket() {Title = "1. Define Website Purpose and Content Structure", Description = "Determine the goals of the website (e.g., showcase work, blog, resume). Organize sections like \"About Me,\" \"Portfolio,\" \"Resume,\" \"Contact,\" and possibly a blog.", Created = DateTimeOffset.Now, ProjectId = portfolioId, TicketPriorityId = priorityLow, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "2. Design Website Layout and User Interface", Description = "Create wireframes or mockups for the site layout using design tools (e.g., Figma, Adobe XD). Plan the color scheme, typography, and overall visual style to match brand.", Created = DateTimeOffset.Now, ProjectId = portfolioId, TicketPriorityId = priorityMedium, TicketStatusId = statusResolved, TicketTypeId = typeChangeRequest},
                              new Ticket() {Title = "3. Choose a Technology Stack", Description = "Decide whether to use HTML/CSS/JavaScript or frameworks like React or Vue.js. Select any additional tools (e.g., CSS frameworks like Bootstrap, Tailwind CSS).", Created = DateTimeOffset.Now, ProjectId = portfolioId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeEnhancement},
                              ...
                              //BLOG
                              new Ticket() {Title = "1. Define Project Scope and Requirements", Description = "Identify the main purpose of the blog (e.g., personal, professional, niche topic). Determine the target audience and content style (e.g., articles, tutorials, opinion pieces).", Created = DateTimeOffset.Now, ProjectId = blogId, TicketPriorityId = priorityLow, TicketStatusId = statusResolved, TicketTypeId = typeDefect},
                              new Ticket() {Title = "2. Choose a Platform or Technology Stack", Description = "Decide whether to use a CMS (e.g., WordPress, Ghost) or custom development (e.g., React, Next.js). Select a database solution (if applicable) for storing blog content (e.g., MySQL, MongoDB).", Created = DateTimeOffset.Now, ProjectId = blogId, TicketPriorityId = priorityMedium, TicketStatusId = statusResolved, TicketTypeId = typeEnhancement},
                              new Ticket() {Title = "3. Design Website Layout and User Interface", Description = "Create wireframes or mockups for key pages (e.g., homepage, blog post page, categories). Select color schemes, fonts, and branding elements.", Created = DateTimeOffset.Now, ProjectId = blogId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeChangeRequest},
                              ...
                              //BUGTRACKER                                                                                                                         
                              new Ticket() {Title = "1. Define Project Scope and Requirements", Description = "Identify key features for the issue tracking application (e.g., issue creation, assignment, comments, reporting). Define user roles (e.g., Admin, User, Manager) and their permissions. Establish success metrics for the application (e.g., issue resolution time, user engagement).", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "2. Set Up Development Environment", Description = "Install the necessary SDKs for ASP.NET MVC. Set up the local development environment with SQL Server or another database of choice.", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "3. Create Project Repository and Version Control", Description = "Initialize a Git repository for version control. Set up collaboration tools (e.g., GitHub, GitLab). Define a branching strategy (e.g., feature branches, main branch).", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "4. Design Database Schema", Description = "Create a database schema for issues, users, comments, and roles. Define relationships between tables (e.g., Users <-> Issues, Issues <-> Comments). Set up Entity Framework (EF) or another ORM for data access.", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "5. Implement User Authentication and Authorization", Description = "Use ASP.NET Identity for user authentication (e.g., login, registration). Implement role-based access control (RBAC) to manage user roles and permissions. Enable password recovery and email verification.", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "6. Set Up Project Structure in ASP.NET MVC", Description = "Create the basic structure for the ASP.NET MVC project (Controllers, Views, Models). Set up routing for key pages (e.g., Dashboard, Issues, Profile). Implement basic layout and common elements (e.g., navigation bar, footer).", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              new Ticket() {Title = "7. Design the User Interface (UI)", Description = "Design wireframes or mockups for key pages (e.g., Issue Dashboard, Issue Detail Page, User Profile). Choose a UI framework (e.g., Bootstrap, Tailwind CSS) for styling and responsiveness. Create reusable UI components (e.g., buttons, cards, modals).", Created = DateTimeOffset.Now, ProjectId = bugtrackerId, TicketPriorityId = priorityHigh, TicketStatusId = statusResolved, TicketTypeId = typeNewDev},
                              ...
                              //MOVIE
                              new Ticket() {Title = "1. Define Project Scope and Requirements", Description = "Define core features for the application, such as movie details input, movie poster uploads, and cast/crew management. Identify roles (e.g., Admin, User) and permissions for managing movie data. Establish metrics and goals (e.g., number of movies added, user interactions).", Created = DateTimeOffset.Now, ProjectId = movieId, TicketPriorityId = priorityLow, TicketStatusId = statusResolved, TicketTypeId = typeDefect},
                              ...
                              new Ticket() {Title = "9. Implement Search Functionality for Movies", Description = "Create a search bar for users to find movies by title, genre, or year of release. Implement advanced filtering (e.g., search by cast, crew, or genre). Optimize search performance with indexing or full-text search in PostgreSQL.", Created = DateTimeOffset.Now, ProjectId = movieId, TicketPriorityId = priorityLow, TicketStatusId = statusResolved,  TicketTypeId = typeDefect},
